...
def extract_op_arg(op):
    ...
    for arg in args:
        ...
        attrs = re.sub(", storage_offset: [\da-f]+", "", attrs)
        attrs = re.sub(", requires_grad: false", "", attrs)
        attrs = re.sub(", requires_grad: true", "", attrs)
...
def unique_ops(op_infos):
    ...
    for op_info in op_infos:
        ...
        # print("#######",op_info["args"])
        # index = op_info["args"].remove('storage_data_ptr:')
        # if index != -1:  # 如果找到了目标子字符串
        #     op_info["args"] = op_info["args"][:index] + op_info["args"][index+13:]
